...
export default function FacturasCompraTab() {
  ...
  async function refresh() {
    setFacturas(await getFacturasCompra());
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => setShowCreate(true)} className="flex items-center gap-1.5 bg-sky-600 hover:bg-sky-700 text-white text-sm font-medium rounded-md px-3 py-2">
          <Plus className="w-4 h-4" /> Nueva factura de compra
        </button>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map((f) => (
              <tr key={f.id} onClick={() => setSelectedId(f.id)} className="hover:bg-slate-50 cursor-pointer">
                ...
                <td className="px-4 py-2.5 text-slate-600">{f.proveedor}</td>
...
function CreateFacturaModal({ onClose, onCreated }) {
  const [form, setForm] = useState({ proveedor: '', fechaEmision: new Date().toISOString().slice(0, 10), fechaVencimiento: '', montoNeto: '' });
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 px-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-200">
          <h2 className="text-sm font-semibold text-slate-800">Nueva factura de compra</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3">
          <label className="block">
            ...
            <input required value={form.proveedor} onChange={(e) => set('proveedor', e.target.value)} className="input" />
          </label>
...
function FacturaDrawer({ factura, cuentas, onClose, onChanged }) {
  ...
  async function handlePago(e) {
    ...
    setSaving(true);
    try {
      await registrarPagoProveedor(factura.id, {
  ...
  async function handleNC(e) {
    ...
    setSaving(true);
    try {
      await crearNotaCreditoCompra(factura.id, { fecha: form.get('fecha'), motivo: form.get('motivo'), monto: form.get('monto') });
      setShowNC(false);
      onChanged();
    } catch (err) {
  ...
  async function handleAnular() {
    ...
    setSaving(true);
    try {
      await anularFacturaCompra(factura.id, 'Anulada manualmente');
      onChanged();
    } finally {
      setSaving(false);
    }
      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-xl overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-200 sticky top-0 bg-white">
          <div>
            ...
            <p className="text-xs text-slate-500">{factura.proveedor}</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700"><X className="w-4.5 h-4.5" /></button>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          {showPago && (
            <form onSubmit={handlePago} className="border border-slate-200 rounded-lg p-4 space-y-2">
              <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Registrar pago realizado</p>
          ...
          {showNC && (
            <form onSubmit={handleNC} className="border border-slate-200 rounded-lg p-4 space-y-2">
              <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Registrar nota de crédito</p>
              <input type="date" name="fecha" required defaultValue={new Date().toISOString().slice(0, 10)} className="input" />
              ...
              <button type="submit" disabled={saving} className="w-full bg-slate-700 hover:bg-slate-800 disabled:bg-slate-300 text-white text-sm font-medium rounded-md py-2">{saving ? 'Guardando...' : 'Registrar nota de crédito'}</button>
            </form>
          )}
          ...
          {factura.pagos.length > 0 && (
            <div>
              <p className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">
                <Receipt className="w-3.5 h-3.5" /> Pagos realizados
              </p>
              <ul className="space-y-1.5">
                {factura.pagos.map((p) => (
