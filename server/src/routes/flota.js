...
flotaRouter.use(requireAuth, resolveTenant);
...
flotaRouter.get('/vehiculos', V, async (req, res) => {
  const vehiculos = await req.prisma.vehiculo.findMany({
...
flotaRouter.get('/vehiculos/alertas', V, async (req, res) => {
  const vehiculos = await req.prisma.vehiculo.findMany({ where: { estado: { not: 'Fuera de servicio' } } });
  const alertas = [];
...
flotaRouter.post('/vehiculos', C, async (req, res) => {
  ...
  const vehiculo = await req.prisma.vehiculo.create({
    data: {
...
flotaRouter.patch('/vehiculos/:id', V, async (req, res) => {
  ...
  const vehiculo = await req.prisma.vehiculo.update({
...
flotaRouter.post('/vehiculos/:id/estado', V, async (req, res) => {
  ...
  const before = await req.prisma.vehiculo.findUnique({ where: { id: req.params.id } });
...
flotaRouter.post('/vehiculos/:id/mantencion', E, async (req, res) => {
  ...
  const before = await req.prisma.vehiculo.findUnique({ where: { id: req.params.id } });
  if (!before) return res.status(404).json({ error: 'Vehículo no encontrado' });
  if (!(await tienePermiso(req, 'flota', 'ver')) && before.tecnico !== req.user.nombre) {
